...
const newCreditRequest = (data) => {
  axios.put("http://localhost:3000/api/user?process=nc", data).then((res) => {
    if (res.status === 200) alert("Успешно");
    else alert("Ошибка при попытке создания пользователя ");
  });
};

const NewCredit = ({ closeHandler = () => {} }) => {
  ...
  return (
    <div className="relative px-16 py-4 flxcol bg-white border-2 border-solid border-[#2a72f8] rounded-md">
      <h1 className="font-bold text-xl">Оформить кредит</h1>
      ...
      <div className="flxcol w-full my-2">
        <label className="mb-1">Код из смс клиента</label>
        <input type="number" className="input-def" />
      </div>
      <button
        className=" bg-[#2a72f8] rounded-md shadow-md py-2 mt-4 text-white"
        onClick={() => {
          newCreditRequest(formData);
          closeHandler(null);
        }}
      >
        Начислить
